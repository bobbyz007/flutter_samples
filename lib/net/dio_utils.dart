...
class DioUtils {
  ...
  static DioUtils _getInstance() {
    return _instance ??= DioUtils._internal();
  }
  ...
  DioUtils._internal() {
    ...
    options.receiveTimeout = const Duration(milliseconds: 2 * 60 * 1000);
    options.sendTimeout = const Duration(milliseconds: 10 * 1000);
    // 初始化
    _dio = Dio(options);
  ...
  Future<ResponseInfo> getRequest(
    ...
    try {
      Response response = await _dio!
          .get(url, queryParameters: queryParameters, cancelToken: cancelTag);
      //响应数据
